...
class Book:
    ...
    def __eq__(self, other):
        """ Overrides the Python == operator so one book can be tested for equality to another book based on
        attribute values """
        if isinstance(self, other.__class__):
            return self.id == other.id and self.title == other.title and self.author == other.author and self.read \
                   == other.read
        return False
    ...
    def __ne__(self, other):
        ...
        return self.id != other.id or self.title != other.title or self.author != other.author or self.read \
               != other.read
...
class BookStore:
    ...
    class __BookStore:
        ...
        def __init__(self):
            create_table_sql = 'CREATE TABLE IF NOT EXISTS books (title TEXT, author TEXT, read BOOLEAN, UNIQUE( ' \
        ...
        # method names prefaced by _ indicate that they are only to be used internally. There's nothing stopping
        # anything else calling _add_book and _update_book but it would go against the intentions of the program to
        # do so. Add_book and update book are called by the Book class's save method, and are used to create or update
        # a book's info in the database. When the program works with books, to save or update, a new Book object is
        # created and the save method is called, for example book = Book('Author', 'Title')  book.save() Or to modify
        # book.title = 'Another Author' book.save()  Operations on the database where the user may not have a book
        # object, have 'public' names like delete_all_books() or get_book_by_id
        ...
        def book_search(self, term):
            ...
            Makes partial matches, so a search for 'row' will match a book with author='JK Rowling' and a book with
            title='Rowing For Dummies'
            :param term the search term
            :returns a list of books with author or title that match the search term. The list will be empty if there
            are no matches.
            """
